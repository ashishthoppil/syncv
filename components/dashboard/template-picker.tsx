...
const SAMPLE_RESUME: ResumeData = {
  ...
  // Only the templates whose region expects these (Europass, Middle East…)
  // render them, so the thumbnails show each convention honestly.
  personal: {
    nationality: "American",
    dateOfBirth: "14/03/1992",
    maritalStatus: "Single",
    workAuthorization: "US Citizen",
    drivingLicence: "Full, clean",
  },
};
...
const TEMPLATE_GROUPS: { id: ResumeTemplateGroup; label: string }[] = [
  { id: "regional", label: "Country formats" },
...
export const ResumeTemplatePicker = ({
  ...
  <div className="space-y-4">
    {TEMPLATE_GROUPS.map((group, index) => {
      const templates = RESUME_TEMPLATE_CONFIGS.filter(
        (template) => template.group === group.id
      );
      const isLast = index === TEMPLATE_GROUPS.length - 1;
      return (
        <div key={group.id}>
          <p className="mb-2 text-[11px] font-medium text-slate-500">{group.label}</p>
